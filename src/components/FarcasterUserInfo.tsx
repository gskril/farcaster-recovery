...
const FarcasterUserInfo = ({ address }: Props) => {
  ...
  const fetchUrlByUsername = username.trim() ? `/api/user-by-username?username=${username.trim()}` : undefined;
  const fetchUrlByCustody = !username.trim() && address ? `/api/user-by-custody?custody_address=${address}` : undefined;

  const { data: userDataByUsername, error: errorByUsername, loading: loadingByUsername } = useFetch<User>(fetchUrlByUsername);
  const { data: userDataByCustody, error: errorByCustody, loading: loadingByCustody } = useFetch<User>(fetchUrlByCustody);
  ...
  useEffect(() => {
    if (!username.trim() && address) {
      ...
      const fetchCustodyData = async () => {
        const response = await fetch(`/api/user-by-custody?custody_address=${address}`);
        if (response.ok) {
  ...
  if (!address && !username.trim()) {
    return <Typography color="red">Please provide a username or connect your address.</Typography>;
  }

  return (
    <Card title={`@${user?.username || ''} - ${user?.displayName || ''}`}>
      <CardDescription>
        Enter a Farcaster username or connect your Ethereum address to fetch user data.
      </CardDescription>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem', width: '100%' }}>
        ...
        {user && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px' }}>
            <img src={user.pfpUrl} alt="Profile" style={{ width: '100px', height: '100px', borderRadius: '50%' }} />
            <Typography onClick={() => copyToClipboard(`${user.fid}`)} style={{ cursor: 'pointer' }}>{`FID: ${user.fid}`}</Typography>
            <Typography onClick={() => copyToClipboard(user.custodyAddress)} style={{ cursor: 'pointer' }}>{`Custody Address: ${user.custodyAddress}`}</Typography>
            <Typography onClick={() => copyToClipboard(recoveryAddress || '')} style={{ cursor: 'pointer' }}>{`Recovery Address: ${recoveryAddress || 'Not Set'}`}</Typography>
            <div style={{ background: '#e6f4ff', padding: '10px', borderRadius: '5px', width: '100%' }}>
            ...
            {copied && <Typography style={{ color: '#0070f3' }}>Copied to clipboard!</Typography>}
          </div>
        )}
      </div>
    </Card>
